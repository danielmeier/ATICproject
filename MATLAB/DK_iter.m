...
figure
subplot(2,3,1)
loglog(Dl0(1,1),'r-',Dl0(2,2),'b-',Dl0(3,3),'g-')
grid
xlabel('Frequency [rad/sec]');
legend('D0(1,1)','D0(2,2)','D0(3,3)')
...
DMDi_b = DMDi;
DMDi_b(1,:) = D0_2b_f*DMDi_b(1,:)/D0_1;   
DMDi_b(:,1) = D0_1*DMDi_b(:,1)/D0_2b_f;
bnd_2b = svd(DMDi_b);
bnd_2b = bnd_2b(1);              % select max svd

DMDi_c = DMDi;
DMDi_c(1,:) = D0_2c_f*DMDi_c(1,:)/D0_1;   
DMDi_c(:,1) = D0_1*DMDi_c(:,1)/D0_2c_f;
bnd_2c = svd(DMDi_c);
bnd_2c = bnd_2c(1);              % select max svd

DMDi_d = DMDi;
DMDi_d(1,:) = D0_2d_f*DMDi_d(1,:)/D0_1;   
DMDi_d(:,1) = D0_1*DMDi_d(:,1)/D0_2d_f;
bnd_2d = svd(DMDi_d);
bnd_2d = bnd_2d(1);              % select max svd


subplot(2,3,6)
semilogx(muRP(2),'k-',bnd_2a,'b-',bnd_2b,'g-',...
         bnd_2c,'r-',bnd_2d,'c-')
xlabel('Frequency [rad/sec]')
grid
